...
# This file lives in pipelines/miniscope/common/. Resolve sibling
# directories relative to it so it works no matter where the pipeline root
...
# repo_root/common -- shared job_template.sh/monitor_resources.sh, sourced
# by pipeline_common.sh.
export REPO_COMMON_DIR="$(cd "$CAIMAN_ROOT_DIR/../../common" && pwd)"

# `run` (the user-facing CLI, see cli/README.md) lives at the repo root's
# cli/, two levels up from pipelines/miniscope/ (repo_root/pipelines/miniscope
# -> repo_root/cli). Adding it to PATH here means it's deployed and updated
# the exact same way as everything else -- through the `current` symlink --
