...
# by every stage script (see extract.sbatch etc.).
export REPO_COMMON_DIR="$(cd "$MOSEQ_ROOT_DIR/../../common" && pwd)"

# Same cli/ wiring as Miniscope's env_setup.sh -- idempotent, safe if both
# pipelines' env_setup.sh get sourced in the same shell. MOSEQ_ROOT_DIR is
# repo-root/pipelines/moseq, so cli/ (a repo-root sibling of pipelines/) is
# two levels up.
...
# extract.sbatch/pca_*.sbatch job scripts in extract/ and pca/.
apptainer_exec() {
  apptainer exec --env "RCLONE_CONFIG=${RCLONE_CONFIG}" --env "PYTHONNOUSERSITE=1" \
    "$MOSEQ_SIF" "$@"
}
