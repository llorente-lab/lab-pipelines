#!/bin/bash
# Environment setup for the <name> pipeline. Not `set -e` -- this gets
# sourced into an ordinary interactive shell as well as non-interactively
# (kernel wrappers, `run` itself), so don't assume a TTY.
#
# Add to ~/.bashrc (setup.sh does this for you once this pipeline has a
# real entry in the repo root's pipelines.yaml):
...
# Where this pipeline's projects/sessions/outputs live -- the
# required_env_var pipelines.yaml checks for and `run` gates on. Pick
# whatever name and default fit your pipeline's data model.
