#!/bin/bash
# Dry-run validator for the --exclusive/--cores/--mem/--time overrides and
# the new qos wiring, without submitting or running anything real.
#
...
#                    (e.g. "Requested time limit is invalid" means the QOS
#                    doesn't allow that walltime -- exactly the failure mode
#                    the qos: high_p fix was for).
#
...
_header "MOSEQ: kappa-scan (this is the one that NEEDS qos=high_p -- default QOS's 2-day MaxWall would otherwise reject the 7-day request)"
...
_run run moseq kappa-scan "$TEST_PROJECT" --n-models 5 --time 10-00:00:00   # deliberately over even high_p's 7-day cap -- SHOULD be rejected

_header "MOSEQ: learn-model (also needs qos=high_p -- default 3-day request exceeds normal QOS's 2-day cap)"
